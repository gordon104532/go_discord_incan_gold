...
func 探險者動作(探險者名稱, 動作 string) {
	探險者動作錯誤 = ""
	if 遊戲狀態 != 3 {
		探險者動作錯誤 = "動作失敗，目前遊戲狀態:" + 遊戲狀態表[遊戲狀態] + "  💦\n"
		fmt.Print(探險者名稱, " ", 探險者動作錯誤)
		return
	}
	...
	if 探險隊[探險者名稱].冒險狀態 == 1 {
		...
		探險隊[探險者名稱] = 探險者異動
		fmt.Println("收到動作: ", 探險者名稱, " ", 動作, " ", 探險隊[探險者名稱])
		已出牌人數++
	} else {
		探險者動作錯誤 = "動作失敗，探險者-冒險狀態:" + 冒險者狀態表[探險隊[探險者名稱].冒險狀態] + "  💦\n"
		fmt.Print(探險者名稱, " ", 探險者動作錯誤)
		return
	}
...
func 回合初始化() {
	...
	fmt.Println("===回合初始化=== ", 回合)
	fmt.Printf("回合卡池: %d, 卡號: %d\n", len(回合卡池), 抽出卡號)
	fmt.Println(回合卡池)
	...
	if strings.Contains(抽出卡片, "寶藏") || strings.Contains(抽出卡片, "神器") {
		...
		if 寶藏鑽石數, 有寶藏 := 寶藏[抽出卡片]; 有寶藏 {
			...
			場上閒置鑽石 = 回合總鑽石數
			公告內容 = 公告內容 + fmt.Sprintf("%d人平分, 剩 %d 鑽\n", 探險中人數, 場上閒置鑽石)
		}
	} else {
		for 探險者名稱 := range 探險隊 {
			var 探險者異動 探險者 = 探險隊[探險者名稱]
...
func 回合結算() {
	// 探險隊結算
	fmt.Println("===回合結算=== ", 回合)
	fmt.Println("探險中人數:", 探險中人數, "回合撤退人數", 回合撤退人數)
	遊戲狀態 = 4
